...
///
/// Contrairement aux écrans Paiement/Discipline/Inscription, il n'y a pas
/// ici de notion de "section active unique" à basculer : l'éligibilité
/// d'un élève à un frais est entièrement déterminée par le PÉRIMÈTRE DU
/// FRAIS lui-même (scope 'all' / 'section' / 'classe', voir
/// `_isEligible`), pas par une section affichée à l'écran. On élargit
/// donc simplement le pool d'élèves visibles à l'UNION de toutes les
/// sections accordées par la clé (`assignedSections`), et on laisse le
/// filtre d'éligibilité du frais sélectionné faire le tri — exactement
/// comme le fait déjà AutresFraisScreen côté admin (qui voit toute
/// l'école).
///
/// Chaque paiement est envoyé au serveur via
/// /school/submit_autre_frais_payment et reste EN ATTENTE tant que
/// l'admin ne l'a pas validé (POST
/// /school/validate_autres_frais_payments) — exactement la même règle
/// que pour les frais mensuels (clé PAY).
///
...
class SubUserAutresFraisScreen extends StatefulWidget {
  ...
  // ⚡⚡⚡ NOUVEAU — liste complète des sections accordées par la clé. Les
  // élèves de TOUTES ces sections sont potentiellement visibles ; c'est
  // le périmètre (`scope`) de chaque "autre frais" qui filtre ensuite qui
...
class _SubUserAutresFraisScreenState extends State<SubUserAutresFraisScreen> {
  ...
  Future<void> _fetchData() async {
    // 1) Élèves (toutes sections accordées par la clé) + paiements déjà
    //    validés (via /restore : données complètes de l'école, incluant
  ...
  void _applyRestoreData(Map<String, dynamic> data) {
    ...
    // ⚡⚡⚡ NOUVEAU — union de toutes les sections accordées par la clé,
    // au lieu d'une seule section fixe.
    final filtered = rawList
        .map((e) => Map<String, dynamic>.from(e as Map))
        .where((e) => widget.assignedSections.contains(e['section']))
        .where((e) =>
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          "Confirmer le paiement de "
              ...
              "pour ${eleve['nom']} ${eleve['prenom']} (${eleve['classe']} — ${eleve['section']}) ?",
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    // ⚡⚡⚡ NOUVEAU — pas de "section active" ici : le titre affiche
    // l'ensemble des sections accordées par la clé (ou la classe unique
    // si la clé y est verrouillée).
    final String titleSuffix = widget.assignedClasse != null
        ? "${widget.assignedSections.join(' + ')} - ${widget.assignedClasse}"
        : widget.assignedSections.join(' + ');
