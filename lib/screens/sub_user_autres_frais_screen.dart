...
/// ⚡⚡⚡ Écran "Autres Frais" pour un sous-utilisateur (clé AFR), capable
...
/// travail alloué par l'admin est écoulé, même en cours d'utilisation.
...
class _SubUserAutresFraisScreenState extends State<SubUserAutresFraisScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final response = await http
          .get(Uri.parse(
          '$base/school/get_autres_frais?school_code=${widget.schoolCode}'))
          .timeout(const Duration(seconds: 15));
  ...
  Future<bool> _trySubmit(Map<String, dynamic> payload) async {
    try {
      ...
      final response = await http
          .post(
        Uri.parse('$base/school/submit_autre_frais_payment'),
