...
/// ⚡⚡⚡ Écran Discipline pour un sous-utilisateur (clé DISC), capable de
/// couvrir PLUSIEURS sections/options. Toutes les requêtes passent
/// désormais par NetworkResolver.resolve() : point d'accès local de
/// l'admin en priorité, internet en repli.
///
/// ⚡ NOUVEAU — se déconnecte automatiquement dès que le temps de
/// travail alloué par l'admin (voir `scheduleSessionExpiry` dans
/// main.dart) est écoulé, même en cours d'utilisation de cet écran.
...
class _SubUserDisciplineScreenState extends State<SubUserDisciplineScreen> {
  ...
  Future<void> _loadExistingAttendance() async {
    ...
    try {
      ...
      final response = await http
          .get(Uri.parse(
          '$base/school/get_attendance?school_code=${widget.schoolCode}'
  ...
  Future<void> _sendAbsences() async {
    ...
    try {
      ...
      final response = await http
          .post(
        Uri.parse('$base/school/record_absences'),
            ...
            ElevatedButton(
              ...
              onPressed: sending
                ...
                try {
                  ...
                  final response = await http
                      .post(
                    Uri.parse('$base/school/send_convocation'),
                    headers: {'Content-Type': 'application/json'},
                    body: jsonEncode({
                      'school_code': widget.schoolCode,
                      'student_id': eleve['id'],
            ...
            ElevatedButton(
              onPressed: sending
                ...
                try {
                  ...
                  final response = await http
                      .post(
                    Uri.parse('$base/school/send_announcement'),
                    headers: {'Content-Type': 'application/json'},
