...
/// L'utilisateur peut basculer entre elles via le sélecteur (SectionSwitcher)
/// dans l'AppBar, exactement comme côté paiement (SubUserHomeScreen).
///
/// Rien ici ne passe par une file d'attente de validation admin :
/// absences, convocations et communiqués partent DIRECTEMENT aux parents
/// dès l'envoi — via /school/record_absences, /school/send_convocation et
...
/// autorisé par la clé (la section active, la classe si elle est
/// verrouillée, ou l'ensemble des sections accordées) : jamais "toute
/// l'école". Cette limite n'est PAS vérifiée côté serveur —
/// /school/send_announcement fait confiance à ce que le client envoie.
...
class _SubUserDisciplineScreenState extends State<SubUserDisciplineScreen> {
  ...
  // ⚡⚡⚡ NOUVEAU — section actuellement active (bascule).
  ...
  // ⚡⚡⚡ NOUVEAU — refiltre localement à partir des données déjà en
  // cache, selon la section ACTIVE. Utilisé aussi bien au premier
  // chargement qu'à chaque bascule de section.
  ...
  Future<void> _switchSection(String newSection) async {
    ...
    setState(() {
      ...
      selectedClasse = widget.assignedClasse; // reste verrouillé si applicable
      absentIds.clear();
      isLoading = true;
    });
  ...
  // dans la classe affichée, la classe elle-même, la section active, ou
  // (⚡⚡⚡ NOUVEAU) l'ENSEMBLE des sections accordées par la clé lorsqu'elle
  // en couvre plusieurs. Jamais "toute l'école".
  // ==========================================================================
  void _showCommuniqueDialog() {
    final titleController = TextEditingController(text: "Communiqué de l'école");
    final messageController = TextEditingController();
    final bool hasMultipleSections = widget.assignedSections.length > 1;
                ...
                children: [
                  ...
                  if (widget.assignedClasse == null)
                    RadioListTile<String>(
                      ...
                      value: 'section',
                      groupValue: target,
                      title: Text("Toute la section active ($activeSection)"),
                      onChanged: (v) => setDialogState(() => target = v!),
                    ),
                  // ⚡⚡⚡ NOUVEAU — cible toutes les sections accordées par
                  // la clé d'un seul coup (nécessite plusieurs sections).
                  if (widget.assignedClasse == null && hasMultipleSections)
                    RadioListTile<String>(
                      dense: true,
                      value: 'sections',
                      groupValue: target,
                      title: Text(
                          "Toutes mes sections (${widget.assignedSections.join(', ')})"),
                      onChanged: (v) => setDialogState(() => target = v!),
                    ),
                  RadioListTile<String>(
                    dense: true,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: sending
                ...
                if (target == 'sections') {
                  payload['sections'] = widget.assignedSections;
                }
                if (target == 'students') {
