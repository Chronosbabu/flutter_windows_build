...
// ====================================================================
// ⚡ NOUVEAU — CONFIGURATION MANUELLE DE L'ADRESSE DU SERVEUR (secours)
//
// La détection automatique (diffusion UDP vers le petit serveur de
// découverte de l'admin, voir NetworkResolver) trouve normalement le
// serveur local toute seule, sans aucune saisie. Ce réglage n'est
// qu'un FILET DE SÉCURITÉ pour les rares réseaux qui bloquent les
// diffusions UDP (certains routeurs/points d'accès avec "isolation des
// clients" activée) : l'agent peut alors indiquer une fois l'adresse
// exacte communiquée par l'admin, qui sera retentée automatiquement si
// la détection échoue — voir NetworkResolver._detect().
// ====================================================================
Future<void> showManualServerIpDialog(BuildContext context) async {
  final controller = TextEditingController();
  final currentIp = await NetworkResolver.getManualIp();
  ...
  await showDialog(
    context: context,
    builder: (ctx) => AlertDialog(
      title: const Text("Adresse du serveur (secours)"),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text(
            "La détection automatique du serveur local fonctionne dans "
                "la grande majorité des cas — vous n'avez normalement "
                "rien à saisir ici.\n\n"
                "N'utilisez ce champ que si l'admin vous a explicitement "
                "communiqué une adresse IP fixe et que la connexion "
                "automatique ne fonctionne pas chez vous.",
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: "Adresse IP (ex: 192.168.4.200)",
      ...
      actions: [
        TextButton(
          onPressed: () async {
            await NetworkResolver.setManualIp(null);
            if (ctx.mounted) Navigator.pop(ctx);
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Adresse de secours effacée")),
              );
            }
          },
        ...
        ElevatedButton(
          onPressed: () async {
            await NetworkResolver.setManualIp(controller.text.trim());
            if (ctx.mounted) Navigator.pop(ctx);
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("Adresse de secours enregistrée")),
              );
            }
          },
...
class _KeyLoginScreenState extends State<KeyLoginScreen> {
  ...
  Future<void> _loginWithKey() async {
    ...
    try {
      // On résout automatiquement l'adresse à contacter : réseau local
      // découvert automatiquement (point d'accès de l'admin, ESP32 ou
      // autre) s'il est joignable, sinon internet. Aucune saisie
      // manuelle d'IP n'est nécessaire dans le cas normal.
          ...
          children: [
            ...
            const Text(
              "Une même clé donne accès à un seul usage (paiement, "
                  ...
                  "Si vous êtes connecté au même réseau WiFi que l'admin, "
            ...
            TextButton.icon(
              ...
              label: const Text(
                "Adresse du serveur (secours, optionnel)",
                style: TextStyle(fontSize: 12.5),
              ),
              onPressed: () => showManualServerIpDialog(context),
            ),
...
// ====================================================================
// ⚡ Petit badge affiché dans l'AppBar quand le réseau LOCAL (point
// d'accès de l'admin, ou ESP32) est utilisé, pour informer visuellement
// le sous-utilisateur qu'il travaille sans internet, en parallèle avec
// les autres agents connectés au même réseau.
// ====================================================================
class LocalModeBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return const Padding(
      ...
      child: Tooltip(
        message: "Connecté au réseau local (sans internet)",
...
// ====================================================================
// ÉCRAN PARAMÈTRES : déconnexion + mode clair/sombre + adresse serveur
// ====================================================================
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Card(
            child: ListTile(
              leading: const Icon(Icons.settings_ethernet, color: Colors.indigo),
              title: const Text("Adresse du serveur (secours)"),
              subtitle: const Text(
                "Normalement inutile — n'y touchez que sur indication de "
                    "l'administrateur",
              ),
              onTap: () => showManualServerIpDialog(context),
            ),
          ),
          const SizedBox(height: 20),
          Card(
